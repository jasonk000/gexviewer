...
function GexSpxScatter({data, recentData}) {
    ...
    const renderArea = () => {
        ...
        const xScale = d3.scaleLinear().domain([GEX_MIN, GEX_MAX]).range([0, width])
        const yScale = d3.scaleLinear().domain([FORWARD_MIN, FORWARD_MAX]).range([height, 0])
        ...
        verticalBarsG.selectAll()
            ...
            .attr('d', d => `M${xScale(d.gex).toFixed()},${height} ${xScale(d.gex).toFixed()},0`)
    }
